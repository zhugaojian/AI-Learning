...
def main():
    ...
    success, message = auth.register_user("demo_user", "demo_password")
    print(f"   注册结果: {success}, 消息: {message}")
    
    # 2. 登录成功测试，使用demo_user登录
    print("\n2. 登录成功测试，使用demo_user登录")
    success, message = auth.login("demo_user", "demo_password")
    print(f"   登录结果: {success}, 消息: {message}")
    
    # 3. 登录失败（密码错误）
    print("\n3. 登录失败测试，使用错误密码登录demo_user")
    success, message = auth.login("demo_user", "wrong_password")
    print(f"   登录结果: {success}, 消息: {message}")
    ...
    for i in range(5):
        success, message = auth.login("demo_user", f"wrong{i}")
